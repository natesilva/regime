...
App.prototype.lastState = function(newState) {
  if (newState) {
    fs.writeFileSync(path.join(this.appDir, 'lastState'), newState + '\n');
  } else {
    var data = fs.readFileSync(path.join(this.appDir, 'lastState'));
  ...
  Seq()
    ...
    .seq(function() {
      ...
      child.on('exit', function(code) {
        ...
        console.error('[' + self.name + '] pid', this.pid, 'exited with code',
          code);
      });

      self.lastState('started');
      console.log('[' + self.name + '] started with pid', self.pid, 'on port',
        self.port);
      callback();
    })
    ['catch'](callback)
